...
const ReportPDFExport = dynamic(
  ...
  { ssr: false, loading: () => <div className="h-10 w-32 animate-pulse rounded-xl dark:bg-white/[0.04] bg-gray-100" /> }
);
...
function KPICard({ kpi, index }: { kpi: KPI; index: number }) {
  ...
  const changeColor =
    kpi.changeType === "positive"
      ...
      : kpi.changeType === "negative"
        ? "dark:text-rose-400 text-rose-600 bg-rose-500/10"
        : "dark:text-white/40 text-gray-500 dark:bg-white/[0.04] bg-gray-100";
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 space-y-2">
        <p className="text-sm font-medium dark:text-white/40 text-gray-500">{kpi.label}</p>
        <p className="text-3xl font-bold tracking-tight dark:text-white text-gray-900">{kpi.value}</p>
      </div>
      {kpi.change && (
...
function LoadingState() {
  return (
    ...
    >
      ...
      <h3 className="mt-6 text-lg font-semibold dark:text-white text-gray-900">AI is preparing your report...</h3>
      <p className="mt-2 text-sm dark:text-white/40 text-gray-500 max-w-sm text-center">
      ...
      <AnimatePresence mode="wait">
        {!report && !loading ? (
          ...
          >
            {/* Header */}
            <div>
              <h1 className="text-3xl font-bold tracking-tight dark:text-white text-gray-900">Reports</h1>
              <p className="dark:text-white/40 text-gray-500 mt-1">
            ...
            >
              <div className="flex items-center gap-3 dark:border-white/[0.08] border-gray-200 border-b px-6 py-4">
                <div className="rounded-xl bg-gradient-to-br from-violet-500 to-indigo-600 p-2.5">
                  <Bot className="h-5 w-5 text-white" />
                </div>
                <div>
                  <h2 className="text-base font-semibold dark:text-white text-gray-900">Executive AI Reports</h2>
                  <p className="text-xs dark:text-white/40 text-gray-500">Powered by AIOps Intelligence</p>
                </div>
              </div>
              ...
              <form onSubmit={handleSubmit} className="p-6">
                <div className="relative">
                  <textarea
                    ...
                    rows={3}
                    className="w-full resize-none rounded-xl dark:border-white/[0.08] border-gray-200 dark:bg-white/[0.04] bg-gray-100 px-5 py-4 pr-36 text-sm dark:text-white/80 text-gray-800 placeholder:text-gray-400 outline-none transition-colors focus:border-violet-500/40 dark:focus:bg-white/[0.06] focus:bg-gray-50 focus:ring-2 focus:ring-violet-500/20"
                  />
                  <button
                    type="submit"
              ...
              {/* Example Chips */}
              <div className="dark:border-white/[0.08] border-gray-200 border-t px-6 py-4">
                <p className="text-xs font-medium dark:text-white/30 text-gray-500 mb-3">Try an example</p>
                <div className="flex flex-wrap gap-2">
                  {EXAMPLE_PROMPTS.map((example) => (
                    <button
                      ...
                      disabled={loading}
                      className="glass-subtle inline-flex items-center gap-2 rounded-full px-4 py-2 text-xs font-medium dark:text-white/50 text-gray-500 transition-all hover:border-violet-500/20 hover:text-violet-400 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
          ...
          >
            ...
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              <div className="flex items-center gap-4">
                ...
                >
                  <ArrowLeft className="h-5 w-5 dark:text-white/40 text-gray-500" />
                </button>
                <div>
                  <h1 className="text-3xl font-bold tracking-tight dark:text-white text-gray-900">
                    {report.title}
                  </h1>
                  <div className="flex items-center gap-3 mt-1 text-sm dark:text-white/40 text-gray-500">
                    <span className="inline-flex items-center gap-1.5 rounded-full bg-violet-500/10 border border-violet-500/20 px-2.5 py-0.5 text-xs font-medium dark:text-violet-400 text-violet-600">
              ...
              >
                <div className="flex items-start gap-3">
                  <div className="rounded-xl bg-violet-500/15 p-2">
                    <Sparkles className="h-4 w-4 dark:text-violet-400 text-violet-600" />
                  </div>
            ...
            >
              <h2 className="text-sm font-semibold dark:text-white text-gray-900 mb-3">Executive Summary</h2>
              <p className="text-sm dark:text-white/50 text-gray-500 leading-relaxed whitespace-pre-line">
                {report.summary}
              </p>
            </motion.div>
