      ...
      <div className="p-5">
        <ResponsiveContainer width="100%" height={280}>
          <LineChart data={chart.data}>
            <CartesianGrid strokeDasharray="3 3" stroke="oklch(0.922 0.004 285.823)" />
            <XAxis dataKey="name" tick={{ fontSize: 12 }} stroke="oklch(0.556 0.005 285.823)" />
            <YAxis tick={{ fontSize: 12 }} stroke="oklch(0.556 0.005 285.823)" tickFormatter={(v) => `$${(v / 1000).toFixed(0)}k`} />
          ...
          {chart.data.map((d, i) => {
            ...
            return (
              <div key={d.name} className="flex items-center gap-3">
                <div className="w-32 shrink-0 truncate text-sm text-muted-foreground" title={d.name}>
                  {d.name}
                </div>
