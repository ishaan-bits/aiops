    ...
    <div className="glass flex flex-col rounded-3xl overflow-hidden" style={{ height: "calc(100vh - 280px)", minHeight: "500px" }}>
      ...
      <div className="flex items-center gap-2 border-b border-white/[0.08] px-5 py-3">
        ...
        <div>
          ...
          <p className="text-xs text-white/40">Search across your documents</p>
        </div>
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-5 space-y-5">
        {messages.length === 0 && !loading && (
          <div className="flex flex-col items-center justify-center h-full text-center">
            ...
            <p className="text-xs text-white/40 mb-6">Upload documents to your Knowledge Base, then ask questions here.</p>
            <div className="flex flex-wrap gap-2 justify-center max-w-md">
              {SUGGESTIONS.map((s) => (
                <button
                  key={s}
                  onClick={() => handleSend(s)}
                  className="glass-subtle rounded-lg px-3 py-1.5 text-xs font-medium text-white/40 transition-colors hover:text-white/70"
                >
                  {s}
                </button>
              ))}
        ...
        {messages.map((msg) => (
          <div key={msg.id} className={`flex gap-3 ${msg.role === "user" ? "justify-end" : ""}`}>
            ...
            <div className={`max-w-[75%]`}>
              ...
              >
                {msg.role === "ai" && (
                  <p className="text-xs font-medium text-violet-400 mb-1">AIOps</p>
                )}
                {msg.role === "user" && (
              ...
              {msg.sources && msg.sources.length > 0 && (
                <div className="mt-2 space-y-1.5">
                  <p className="text-xs font-medium text-white/30 px-1">Sources</p>
                  {msg.sources.map((src, i) => (
                    ...
                    >
                      ...
                      <span className="text-xs font-medium text-white/70 truncate">{src.document}</span>
                      <span className="text-xs text-white/20">·</span>
                      <span className="text-xs text-white/30">Page {src.page}</span>
                      <span className="ml-auto text-xs text-white/30">{Math.round(src.score * 100)}%</span>
                    </div>
                  ))}
                </div>
              )}
        ...
        {loading && (
          <div className="flex gap-3">
            ...
            <div className="rounded-2xl rounded-bl-md bg-white/[0.06] px-5 py-3">
              <div className="flex items-center gap-2">
                <Loader2 className="h-4 w-4 text-violet-400 animate-spin" />
                <span className="text-sm text-white/40">Reading documents with AI...</span>
              </div>
            </div>
          </div>
        )}
      ...
      <div className="border-t border-white/[0.08] p-4">
        <div className="flex items-end gap-2">
          <textarea
            ...
            rows={1}
            disabled={loading}
            className="flex-1 resize-none rounded-xl border border-white/[0.08] bg-white/[0.04] px-4 py-2.5 text-sm text-white/80 placeholder:text-white/30 outline-none focus:border-violet-500/40 focus:bg-white/[0.06] focus:ring-2 focus:ring-violet-500/20 disabled:opacity-50"
          />
          <button
            onClick={() => handleSend()}
            disabled={!input.trim() || loading}
            className="rounded-xl bg-gradient-to-br from-indigo-500 to-violet-500 p-2.5 text-white shadow-sm transition-all hover:shadow-md hover:shadow-violet-500/25 disabled:opacity-50 disabled:cursor-not-allowed"
          >
